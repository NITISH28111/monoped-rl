...
class MonopedState(object):
    ...
    def _hop_r_pitch_rate(self):
        """
        Penalty for pitch angular velocity (IMU angular_velocity.y).

        Root cause from sensor data: the IMU showed pitch-rate spikes up to
        480 rad/s — the torso was freely rotating backward with no damping.
        The old reward never touched angular velocity at all, so once the torso
        started tilting nothing pulled it back.

        This term fires on the rotation RATE, complementing orientation_pitch
        which fires on the accumulated angle.  Together they damp both the
        instantaneous spin and the resulting lean.

        Returns a positive number — caller subtracts it.
        """
        pitch_rate = self.base_angular_velocity.y   # rad/s; negative = pitching backward
        # Only penalise backward pitch-rate (positive pitch_rate = leaning forward = good)
        if pitch_rate < 0:
            return HOP_W['pitch_rate'] * pitch_rate ** 2
        return 0.0
